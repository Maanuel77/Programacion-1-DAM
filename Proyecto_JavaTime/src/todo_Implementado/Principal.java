...
public class Principal {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 2:
				// 2. Buscar devolviendo null e imprimiendo en main
			...
			case 3:
				...
				if (java.time.ZoneId.getAvailableZoneIds().contains(zona)) {

					// Si la zona es correcta, creamos el objeto
					Evento nuevo = new Evento(id, nombre, anio, mes, dia, hora, min, zona);

					// Intentamos añadirlo a la agenda (comprobando que no esté repetido)
					...
					// Si la zona no existe, avisamos y no creamos el evento
					System.out.println("Error: La Zona Horaria '" + zona + "' no es válida.");
					System.out.println("Usa el formato 'Continente/Ciudad' (ej. Asia/Tokyo).");
				}
				break;
			...
			case 4:
				...
				if (aModificar != null) {
					...
					if (java.time.ZoneId.getAvailableZoneIds().contains(nuevaZona)) {
						...
						System.out.println(
								"Error: Zona horaria no reconocida. Recuerda usar el formato 'Continente/Ciudad'.");
					}
